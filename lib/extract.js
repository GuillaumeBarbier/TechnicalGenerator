...
function buildShape(cat) {
  const topCells = [
    { label: "RÉFÉRENCE du produit au format 'REF. <code>' (ex: REF. OZ LB 7552)", value: "nom du modèle (identique à name)", enabled: true },
  ...
  const shape = {
    ...
    features: [
      ...
      { label: "MARQUE FRANÇAISE", value: "CHECK" },
    ],
    pack: [
...
function buildSystemPrompt(cat) {
  ...
  return (
    ...
    "RÉFÉRENCE (champ ref) : cherche dans la page le code de référence du produit, souvent introduit par « Référence », « Référence Produit », « REF », « Réf. » ou « SKU » (ex sur la page : « Référence Produit OZ LB 7552 »). " +
    "Recopie le code EXACT et formate-le « REF. <code> » (ex: « REF. OZ LB 7552 »). La 1re cellule de specsTop doit avoir label = cette référence et value = le nom du modèle. Si aucune référence n'est trouvable, laisse ref vide. " +
    ...
    "INCLUS SYSTÉMATIQUEMENT un point fort « MARQUE FRANÇAISE » avec la valeur 'CHECK' (toutes nos marques sont françaises) — de préférence en dernière position. " +
    note +
...
function clean(s) {
  ...
  return s
    ...
    .replace(/\s*\((?:ex|exemple)\s*:[^)]*\)/gi, '') // supprime les "(ex: ...)" recopiés du gabarit
    .replace(/^(?:ex|exemple)\s*:\s*/i, '')  // ou un "ex: ..." en début de valeur
